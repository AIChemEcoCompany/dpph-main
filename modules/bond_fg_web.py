...
# 将SMILES转换为RDKit分子对象的函数
def smiles_to_molecules(smiles_list):
    # 删除分子中每个原子的atom mapping编号
    result = []
...
def draw_molecules(molecules):
    img = Draw._MolsToGridSVG(molecules, molsPerRow=min(4, len(molecules)), subImgSize=(300, 300))
    return img
    ...
    def show_result(self):
        ...
        if self.bond_status:
            for idx, item in enumerate(self.broken_each_reactant_list):
                print("\n【底物{}断键】".format(idx+1))
                svg = self._visualize_mol_with_am(self.am_sub_list[idx])
            ...
            for idx, item in enumerate(self.formed_each_product_list):
                print("\n【产物{}成键】".format(idx+1))
                svg = self._visualize_mol_with_am(self.am_prod_list[idx])
                if item is not None and len(item) > 0:
                    for bond in item:
                        print(bond)
...
class Factory:
    ...
    def get_comb_by_broken_fromed_bond(self, bond, broken_or_formed, sub_idx_list = [], r_count = 1, r_length=1, callback = None):
        # 构造DataFrame，其中包含元素级断键，官能团，反应的可视化图片，断键，反应编号列
        elem_bond = bond[0]
        fg = bond[1]
        relation = bond[2]
        result_list = []
        if broken_or_formed == "broken":
            counter = 0
            for row_idx in tqdm(self.all_broken_dict[bond]["rxn_row_number"]):
                ...
                if self.data[this_row_idx] is not None:
                    if self.data[this_row_idx].am_status == False:
                        ...
                        # new_broken_bond_check.data[this_row_idx].show_result()
                        sub_svg = draw_molecules(smiles_to_molecules(self.data[this_row_idx].am_sub_list))
                        prod_svg = draw_molecules(smiles_to_molecules(self.data[this_row_idx].am_prod_list))

                        # 将SVG内容编码为Base64
                        sub_encoded_svg = base64.b64encode(sub_svg.encode('utf-8')).decode('utf-8')
                        prod_encoded_svg = base64.b64encode(prod_svg.encode('utf-8')).decode('utf-8')

                        # 创建Data URI
                        sub_data_uri = f"data:image/svg+xml;base64,{sub_encoded_svg}"
                        prod_data_uri = f"data:image/svg+xml;base64,{prod_encoded_svg}"
                        ...
                        stat_data = {
                            ...
                            "fg": fg,
                            "relation": relation,
                            "subs_with_am": f'<img src="{sub_data_uri}" alt="SVG Image" />',
                            "prods_with_am": f'<img src="{prod_data_uri}" alt="SVG Image" />',
                        }
                        new_row = pd.Series(stat_data)
                        new_row = pd.DataFrame(new_row).T
                        result_list.append(new_row)
                counter += 1
                process = counter/len(self.all_broken_dict[bond]["rxn_row_number"]) * (1/r_length) + r_count/r_length
                callback(process)
        else:
            for row_idx in self.all_formed_dict[bond]["rxn_row_number"]:
                this_row_idx = int(row_idx) - 1
                # print("反应编号: {}".format(int(row_idx)))
                if self.data[this_row_idx] is not None:
                    if self.data[this_row_idx].am_status == False:
    ...
    def save_stat_df_to_csv(self):
        print("开始保存stat_df数据")
        # 重新排序列名
        new_col_name = ["rxn_idx", "condition_idx", "r_mw<p_mw", "am_status", "bond_status", "rxn_SMILES", "fg_compatibility", "fg_in_prod", "fg_in_sub"]
...
class New_Broken_Bond_Check:
    def __init__(self, project_name):
        ...
        with open("metadata/project_metadata.json", "r", encoding="utf-8") as f:
            self.project_metadata = json.load(f)
            self.project_metadata = self.project_metadata[project_name]

        self.timestamp = time.strftime("%Y%m%d%H%M%S", time.localtime())

        self.df_target = pd.read_excel(self.project_metadata["target"]["rxn_data"]["path"], sheet_name=self.project_metadata["target"]["rxn_data"]["sheet_name"],)
        self.condition_df_target = pd.read_excel(self.project_metadata["target"]["condition_data"]["path"], sheet_name=self.project_metadata["target"]["condition_data"]["sheet_name"])

        self.df_lit = pd.read_excel(self.project_metadata["lit"]["rxn_data"]["path"], sheet_name=self.project_metadata["lit"]["rxn_data"]["sheet_name"],)
        self.condition_df_lit = pd.read_excel(self.project_metadata["lit"]["condition_data"]["path"], sheet_name=self.project_metadata["lit"]["condition_data"]["sheet_name"])
        ...
        self.factory_target = Factory(
            ...
            sub_col_name_list=self.project_metadata["target"]["sub_col_name_list"],
            prod_col_name_list=self.project_metadata["target"]["prod_col_name_list"],
            reagent_col_name_list=self.project_metadata["target"]["reagent_col_name_list"],
            solvent_col_name_list=self.project_metadata["target"]["solvent_col_name_list"],
            cond_id_col_name=self.project_metadata["target"]["cond_id_col_name"],
            other_cond_col_name_list = self.project_metadata["target"]["other_cond_col_name_list"],
            freq_col_name=self.project_metadata["target"]["freq_col_name"],
            data_path = "data/{}".format(project_name)
        )
        self.factory_lit = Factory(
            self.df_lit,
            condition_df = self.condition_df_lit,
            sub_col_name_list=self.project_metadata["lit"]["sub_col_name_list"],
            prod_col_name_list=self.project_metadata["lit"]["prod_col_name_list"],
            reagent_col_name_list=self.project_metadata["lit"]["reagent_col_name_list"],
            solvent_col_name_list=self.project_metadata["lit"]["solvent_col_name_list"],
            cond_id_col_name=self.project_metadata["lit"]["cond_id_col_name"],
            other_cond_col_name_list = self.project_metadata["lit"]["other_cond_col_name_list"],
            freq_col_name=self.project_metadata["lit"]["freq_col_name"],
            data_path = "data/{}".format(project_name)
        )

        # 读入识别和统计结果
        self.factory_target.load_data_from_pickle(self.project_metadata["target"]["pickle_path"])
        self.factory_lit.load_data_from_pickle(self.project_metadata["lit"]["pickle_path"])
        ...
        self.result_dict = {}
        # 找出二级新断键
        for elem_key in self.factory_lit.hierarchical_broken_dict.keys():
            for fg_key in self.factory_lit.hierarchical_broken_dict[elem_key].keys():
                for relation in self.factory_lit.hierarchical_broken_dict[elem_key][fg_key]:
                    if elem_key in self.factory_target.hierarchical_broken_dict.keys():
